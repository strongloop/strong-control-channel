...
function Server(onRequest) {
  ...
  self.on('message', function(message, worker) {
    ...
    if (message.cmd === REQ) {
      ...
      debug('channel request #%d %s', message.seqno, debug.json(request));
      ...
      self.onRequest(request, function(response) {
        debug('channel response #%d %s', message.seqno, debug.json(response));
    ...
    if (message.cmd === RSP) {
      ...
      debug('channel response #%d %s', message.seqno, debug.json(message));
