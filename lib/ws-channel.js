...
var DIS = 'strong-control-channel:disconnect';
...
WebsocketChannel.prototype._attach = function(websocket) {
  ...
  // FIXME commenting out the flush above causes a packet sequence error
  // that I also see ephemerally with the reconnect test.
};
...
WebsocketChannel.prototype._detach = function() {
  ...
  // XXX(sam) should emit on loss of websocket, and possibly include an error
...
WebsocketChannel.prototype.close = function(callback) {
  ...
  // XXX(sam) channel should be marked as closed, so any calls to connect()
  // or request() or notify() cause errors, as well as attempts to _attach()
  // new websockets to this.
...
WebsocketChannel.prototype.destroy = function() {
  if (this._websocket)
    this._closeWebsocket(this._websocket);
  // XXX(sam) mark channel as closed? see .close()
  return this;
};
...
WebsocketChannel.prototype._onOpen = function() {
  this._debug('ws connection open');
...
WebsocketChannel.prototype._handleDisconnect = function() {
  ...
  setTimeout(this._reconnect.bind(this), 1000);
};
