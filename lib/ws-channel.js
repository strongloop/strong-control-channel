// TODO
// - send SHK with error: 'channel closed' when router receives a ws
//   for a known client, with an unknown session
// - send SHK with error: 'unknown client' when router receives a ws
//   for an unknown client
// - distinguish between close and destroy? close should flush messages and
//   close when done, 'graceful', destroy should just close socket, and never
//   reconnect it?
// - check other tests: confirm error and disconnect cases are tested
'use strict';
...
var SHK = 'strong-control-channel:handshake';
//var FIN = 'strong-control-channel:close'; // TODO(sam) an explicit close
...
WebsocketChannel.prototype._detach = function() {
  ...
  // FIXME(sam) should emit on loss of websocket, and possibly include an error
  // message as a reason (if available)
};
...
WebsocketChannel.prototype.request = function(message, callback) {
  ...
  this._debug('send request: id %d %s', requestId, this._debug.json(message));
...
WebsocketChannel.prototype._onOpen = function() {
  ...
  process.nextTick(function() {
    ...
    // XXX(sam) I think both of above should be just 'new-connection', and the
    // websocket shouldn't be emitted, its internal (test-ws-errors is depending
    // on the ws, but it could get it from channel._websocket).
    // XXX(sam) we also need a 'lost-connection' event
  });
...
WebsocketChannel.prototype._handleError = function(err) {
  ...
  setTimeout(this._reconnect.bind(this), 1000); // XXX(bert) make this better.
};


WebsocketChannel.prototype._fatalError = function(err) {
  this.emit('error', err);
  // TODO(bert) detach and close
};
...
WebsocketChannel.prototype._handleDisconnect = function() {
  ...
  this._detach();
  // TODO shouldn't we reconnect, if we didn't get an explicit close?
};
